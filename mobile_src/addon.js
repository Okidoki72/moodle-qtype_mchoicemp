angular.module('mm.addons.qtype_mchoicef', ['mm.core'])
.config(["$mmQuestionDelegateProvider", function($mmQuestionDelegateProvider) {
    $mmQuestionDelegateProvider.registerHandler('mmaQtypemchoicef', 'qtype_mchoicef', '$mmaQtypemchoicefHandler');
}]);
...
.directive('mmaQtypeMultichoice', ["$log", "$mmQuestionHelper", function($log, $mmQuestionHelper) {
	$log = $log.getInstance('mmaQtypemchoicef');
    return {
        restrict: 'A',
        priority: 100,
        templateUrl: 'addons/qtype/mchoicef/template.html',
        link: function(scope) {
...
angular.module('mm.addons.qtype_mchoicef')
.factory('$mmaQtypemchoicefHandler', ["$mmUtil", function($mmUtil) {
    var self = {};
