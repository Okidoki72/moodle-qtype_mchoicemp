...
/**
 * Converts the mchoicef info and writes it into the question.xml
 *
 * @param int $oldversion the old (i.e. current) version of Moodle
 */
function xmldb_qtype_mchoicef_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2011010400) {
        ...
        // Define field correctfeedbackformat to be added to question_mchoicef.
        $table = new xmldb_table('question_mchoicef');
        ...
        // Define field incorrectfeedbackformat to be added to question_mchoicef.
        ...
        if ($dbman->field_exists($table, $field)) {
            ...
            foreach ($rs as $record) {
                ...
                $DB->update_record('question_mchoicef', $record);
            }
            $rs->close();
        }
        // Record that qtype_mchoicef savepoint reached.
        upgrade_plugin_savepoint(true, 2011010400, 'qtype', 'mchoicef');
    }
    ...
    if ($oldversion < 2011011200) {
        ...
        // Define field shownumcorrect to be added to question_mchoicef.
        $table = new xmldb_table('question_mchoicef');
        ...
        // Record that qtype_mchoicef savepoint reached.
        upgrade_plugin_savepoint(true, 2011011200, 'qtype', 'mchoicef');
    }
    ...
    if ($oldversion < 2013110500) {
        ...
        $problemids = $DB->get_recordset_sql("
                SELECT question, MIN(id) AS recordidtokeep
                  FROM {question_mchoicef}
              GROUP BY question
                HAVING COUNT(1) > 1
                ");
        foreach ($problemids as $problem) {
            $DB->delete_records_select('question_mchoicef',
                    'question = ? AND id > ?',
        ...
        // Record that qtype_mchoicef savepoint reached.
        upgrade_plugin_savepoint(true, 2013110500, 'qtype', 'mchoicef');
    }

    if ($oldversion < 2013110501) {

        // Define table question_mchoicef to be renamed to qtype_mchoicef_options.
        $table = new xmldb_table('question_mchoicef');

        // Launch rename table for question_mchoicef.
        if ($dbman->table_exists($table)) {
            $dbman->rename_table($table, 'qtype_mchoicef_options');
        }

        // Record that qtype_mchoicef savepoint was reached.
        upgrade_plugin_savepoint(true, 2013110501, 'qtype', 'mchoicef');
    }

    if ($oldversion < 2013110502) {

        // Define key question (foreign) to be dropped form qtype_mchoicef_options.
        $table = new xmldb_table('qtype_mchoicef_options');
        ...
        // Record that qtype_mchoicef savepoint was reached.
        upgrade_plugin_savepoint(true, 2013110502, 'qtype', 'mchoicef');
    }

    if ($oldversion < 2013110503) {

        // Rename field question on table qtype_mchoicef_options to questionid.
        $table = new xmldb_table('qtype_mchoicef_options');
        ...
        // Record that qtype_mchoicef savepoint was reached.
        upgrade_plugin_savepoint(true, 2013110503, 'qtype', 'mchoicef');
    }

    if ($oldversion < 2013110504) {

        // Define key questionid (foreign-unique) to be added to qtype_mchoicef_options.
        $table = new xmldb_table('qtype_mchoicef_options');
        ...
        // Record that qtype_mchoicef savepoint was reached.
        upgrade_plugin_savepoint(true, 2013110504, 'qtype', 'mchoicef');
    }

    if ($oldversion < 2013110505) {

        // Define field answers to be dropped from qtype_mchoicef_options.
        $table = new xmldb_table('qtype_mchoicef_options');
        ...
        // Record that qtype_mchoicef savepoint was reached.
        upgrade_plugin_savepoint(true, 2013110505, 'qtype', 'mchoicef');
    }

    if ($oldversion < 2015040100) {

        // Fix wrong component for combined feedback files.
        $params = array('component' => 'qtype_mchoicef'
        ...
        // Record that qtype_mchoicef savepoint was reached.
        upgrade_plugin_savepoint(true, 2015040100, 'qtype', 'mchoicef');
    }
