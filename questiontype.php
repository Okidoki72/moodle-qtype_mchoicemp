 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_mchoicef extends question_type {
    ...
    public function get_question_options($question) {
        global $DB, $OUTPUT;
        $question->options = $DB->get_record('qtype_mchoicef_options',
    ...
    public function save_question_options($question) {
        ...
        $options = $DB->get_record('qtype_mchoicef_options', array('questionid' => $question->id));
        if (!$options) {
            $options = new stdClass();
            $options->questionid = $question->id;
            $options->correctfeedback = '';
            $options->incorrectfeedback = '';
            $options->id = $DB->insert_record('qtype_mchoicef_options', $options);
        }
        ...
        $DB->update_record('qtype_mchoicef_options', $options);
    ...
    protected function make_hint($hint) {
        return qtype_mchoicef_hint::load_from_record($hint);
    }
    ...
    public function delete_question($questionid, $contextid) {
        global $DB;
        $DB->delete_records('qtype_mchoicef_options', array('questionid' => $questionid));
    ...
    public function import_from_xml($data, $question, qformat_xml $format, $extra=null) {
        ...
        if (!isset($data['@']['type']) || $data['@']['type'] != 'mchoicef') {
            return false;
        }

        $question = $format->import_headers($data);
        $question->qtype = 'mchoicef';
...
/**
 * An extension of {@link question_hint_with_parts} for mchoicef questions.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_mchoicef_hint extends question_hint_with_parts {
    ...
    public static function load_from_record($row) {
        return new qtype_mchoicef_hint($row->id, $row->hint, $row->hintformat,
