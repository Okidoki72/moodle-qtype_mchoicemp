...
/**
 * Restore handler for Moodle 2.x/3.x mchoicef questions
 *
...
/**
 * Restore plugin class that provides the information needed to restore one mchoicef qtype
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_qtype_mchoicef_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        ...
        $elepath = $this->get_pathfor('/mchoicef');
    ...
    /**
     * Process the qtype/mchoicef element
     *
     * @param stdObject $data question data
     * @return void
     */
    public function process_mchoicef($data) {
        global $DB;
        ...
        if ($questioncreated) {
            ...
            if (!$DB->record_exists('qtype_mchoicef_options', array('questionid' => $data->questionid))) {
                $newitemid = $DB->insert_record('qtype_mchoicef_options', $data);
                $this->set_mapping('qtype_mchoicef_options', $oldid, $newitemid);
            }
        }
    }
     ...
     * recoded pointing to all the restored stuff for mchoicef questions
     *
    ...
    public static function define_decode_contents() {
        ...
        $contents[] = new restore_decode_content('qtype_mchoicef_options',
                $fields, 'qtype_mchoicef_options');
